...
#[derive(Clone)]
pub struct GetResourcesFreeRequest<'a> {
...
impl<'a> GetResourcesFreeRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self.http_client.client.get("/resources/free");
...
impl<'a> ::std::future::IntoFuture for GetResourcesFreeRequest<'a> {
