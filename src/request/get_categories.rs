...
impl<'a> GetCategoriesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Category>> {
        let mut r = self.http_client.client.get("/categories");
...
impl<'a> ::std::future::IntoFuture for GetCategoriesRequest<'a> {
    type Output = httpclient::InMemoryResult<Vec<Category>>;
