...
#[derive(Clone)]
pub struct GetResourcesUpdatesByResourceRequest<'a> {
...
impl<'a> GetResourcesUpdatesByResourceRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<ResourceUpdate>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/resources/{resource}/updates", resource = self.resource));
...
impl<'a> ::std::future::IntoFuture for GetResourcesUpdatesByResourceRequest<'a> {
    type Output = httpclient::InMemoryResult<Vec<ResourceUpdate>>;
