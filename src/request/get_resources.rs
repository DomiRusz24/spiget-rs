...
#[derive(Clone)]
pub struct GetResourcesRequest<'a> {
...
impl<'a> GetResourcesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self.http_client.client.get("/resources");
...
impl<'a> ::std::future::IntoFuture for GetResourcesRequest<'a> {
