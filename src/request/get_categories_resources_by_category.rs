...
#[derive(Clone)]
pub struct GetCategoriesResourcesByCategoryRequest<'a> {
...
impl<'a> GetCategoriesResourcesByCategoryRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/categories/{category}/resources", category = self.category));
...
impl<'a> ::std::future::IntoFuture for GetCategoriesResourcesByCategoryRequest<'a> {
