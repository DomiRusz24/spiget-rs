...
#[derive(Clone)]
pub struct GetSearchResourcesByQueryRequest<'a> {
...
impl<'a> GetSearchResourcesByQueryRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/search/resources/{query}", query = self.query));
        if let Some(ref unwrapped) = self.field {
            r = r.query("field", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetSearchResourcesByQueryRequest<'a> {
