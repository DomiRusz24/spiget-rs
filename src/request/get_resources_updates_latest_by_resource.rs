...
#[derive(Clone)]
pub struct GetResourcesUpdatesLatestByResourceRequest<'a> {
...
impl<'a> GetResourcesUpdatesLatestByResourceRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<ResourceUpdate> {
        let mut r = self
            .http_client
            .client
            .get(
                &format!(
                    "/resources/{resource}/updates/latest", resource = self.resource
                ),
            );
...
impl<'a> ::std::future::IntoFuture for GetResourcesUpdatesLatestByResourceRequest<'a> {
    type Output = httpclient::InMemoryResult<ResourceUpdate>;
