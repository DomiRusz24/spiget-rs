...
#[derive(Clone)]
pub struct GetResourcesPremiumRequest<'a> {
...
impl<'a> GetResourcesPremiumRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self.http_client.client.get("/resources/premium");
...
impl<'a> ::std::future::IntoFuture for GetResourcesPremiumRequest<'a> {
