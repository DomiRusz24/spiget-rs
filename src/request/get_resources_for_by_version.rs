...
#[derive(Clone)]
pub struct GetResourcesForByVersionRequest<'a> {
...
impl<'a> GetResourcesForByVersionRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/resources/for/{version}", version = self.version));
        if let Some(ref unwrapped) = self.fields {
            r = r.query("fields", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.method {
            r = r.query("method", &unwrapped.to_string());
        }
...
impl<'a> ::std::future::IntoFuture for GetResourcesForByVersionRequest<'a> {
    type Output = httpclient::InMemoryResult<Vec<Resource>>;
