...
#[derive(Clone)]
pub struct GetResourcesVersionsByResourceRequest<'a> {
...
impl<'a> GetResourcesVersionsByResourceRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<ResourceVersion>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/resources/{resource}/versions", resource = self.resource));
...
impl<'a> ::std::future::IntoFuture for GetResourcesVersionsByResourceRequest<'a> {
    type Output = httpclient::InMemoryResult<Vec<ResourceVersion>>;
