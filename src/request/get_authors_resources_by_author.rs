...
#[derive(Clone)]
pub struct GetAuthorsResourcesByAuthorRequest<'a> {
...
impl<'a> GetAuthorsResourcesByAuthorRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/authors/{author}/resources", author = self.author));
...
impl<'a> ::std::future::IntoFuture for GetAuthorsResourcesByAuthorRequest<'a> {
