...
#[derive(Clone)]
pub struct GetResourcesNewRequest<'a> {
...
impl<'a> GetResourcesNewRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Resource>> {
        let mut r = self.http_client.client.get("/resources/new");
...
impl<'a> ::std::future::IntoFuture for GetResourcesNewRequest<'a> {
