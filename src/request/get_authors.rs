...
#[derive(Clone)]
pub struct GetAuthorsRequest<'a> {
...
impl<'a> GetAuthorsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Author>> {
        let mut r = self.http_client.client.get("/authors");
...
impl<'a> ::std::future::IntoFuture for GetAuthorsRequest<'a> {
