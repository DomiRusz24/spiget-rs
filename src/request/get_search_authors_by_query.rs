...
#[derive(Clone)]
pub struct GetSearchAuthorsByQueryRequest<'a> {
...
impl<'a> GetSearchAuthorsByQueryRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<Author>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/search/authors/{query}", query = self.query));
...
impl<'a> ::std::future::IntoFuture for GetSearchAuthorsByQueryRequest<'a> {
    type Output = httpclient::InMemoryResult<Vec<Author>>;
