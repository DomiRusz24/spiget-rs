...
#[derive(Clone)]
pub struct GetAuthorsReviewsByAuthorRequest<'a> {
...
impl<'a> GetAuthorsReviewsByAuthorRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<Vec<ResourceReview>> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/authors/{author}/reviews", author = self.author));
...
impl<'a> ::std::future::IntoFuture for GetAuthorsReviewsByAuthorRequest<'a> {
